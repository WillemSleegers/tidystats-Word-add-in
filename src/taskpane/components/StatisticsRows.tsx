...
const StatisticsRows = (props: StatisticsRowsProps) => {
  ...
  const handleCheck = (name: string) => {
    const newItems = items.map((item) => (item.name === name ? { ...item, checked: !item.checked } : item));
    setItems(newItems);
  };
  ...
  const content = items.map((x) => {
    return (
      <Row primary={false} key={x.identifier}>
        <RowName key={x.name} header={false} bold={false} name={x.symbol} subscript={x.subscript} />
        <RowValue key={x.value} value={x.value} />
        <>{clickedSettings && <MyCheckbox checked={x.checked} onChange={() => handleCheck(x.name)} />}</>
        <IconButton iconProps={addIcon} onClick={() => insertStatistic(x.value, x.identifier)} />
      </Row>
    );
  });
