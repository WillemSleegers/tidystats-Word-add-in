...
const insertStatistics = async (statistics: StatisticProps[]) => {
  Word.run(async (context) => {
    ...
    elements = statistics.filter((statistic: StatisticProps) => statistic.checked);

    // Filter out the degrees of freedom if there's a test statistic (e.g., t, F)
    if (elements.some((statistic: StatisticProps) => statistic.name === "statistic")) {
      elements = elements.filter(
        (statistic: StatisticProps) => !["df", "df numerator", "df denominator"].includes(statistic.name)
      );
    }
    ...
    if (lower && upper) {
      elements = elements.filter((statistic: StatisticProps) => statistic.name !== "upper");
    }
    ...
    elements.forEach((statistic: StatisticProps, i: number) => {
      ...
      if (statistic.name === "lower" && lower && upper) {
        ...
        if (statistic.name === "statistic") {
          if (statistic.symbol === "t" && statistics.find((x: StatisticProps) => x.name === "df")) {
            ...
            const dfNum = statistics.find((x: StatisticProps) => x.name === "df numerator");
            if (dfNum) {
              const dfValue = range.getRange("End");
              const dfValueCC = dfValue.insertContentControl();
            ...
            const dfDen = statistics.find((x: StatisticProps) => x.name === "df denominator");
            if (dfDen) {
