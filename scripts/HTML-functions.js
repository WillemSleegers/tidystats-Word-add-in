...
function toggleStatistic() {
  ...
  if (this.parentElement.getAttribute("statistic") == "CI_lower") {
    this.parentElement.nextSibling
      ...
      .classList.toggle("checkbox-selected");
  } else if (this.parentElement.getAttribute("statistic") == "CI_upper") {
    this.parentElement.previousSibling
      .getElementsByClassName("checkbox")[0]
      .classList.toggle("checkbox-selected");
  }
...
function createAnalysis(analyses, identifier) {
  ...
  // Add the method row
  contentContainer = addRow(contentContainer, false, "Method", analysis.method);

  // Add a description row, if there is one
  if ("description" in analysis) {
    contentContainer = addRow(
      contentContainer,
      false,
      "Description",
  ...
  if ("statistics" in analysis) {
    // Add name row
    if ("name" in analysis) {
      contentContainer = addRow(contentContainer, false, "Name", analysis.name);
    }

    contentContainer = addStatisticsRows(
      contentContainer,
      true,
      3,
      analysis.statistics,
  ...
  if ("models" in analysis) {
    contentContainer = addModelsRows(
      contentContainer,
      3,
      analysis.models,
      attrs
  ...
  if ("effects" in analysis) {
    // Add random effects
    contentContainer = addRandomEffectsRows(
      contentContainer,
      3,
      analysis.effects.random_effects,
      attrs
    );

    // Add fixed effects
    contentContainer = addFixedEffectsRows(
      contentContainer,
      3,
      analysis.effects.fixed_effects,
      attrs
    );
  }
  ...
  if ("groups" in analysis) {
    contentContainer = addGroupsRows(
      contentContainer,
      3,
      analysis.groups,
      attrs
...
function formatCIs(CIs) {
  var text =
    "[" + formatNumber(CIs.CI_lower) + ", " + formatNumber(CIs.CI_upper) + "]";
  return text;
}
...
function addTermsRows(element, isParent, level, terms, attrs) {
  ...
  for (var t in terms) {
    ...
    // Add name row
    termsContainer = addRow(termsContainer, true, "Name", term.name);
...
function addGroupsRows(element, level, groups, attrs) {
  ...
  for (var g in groups) {
    ...
    // Add name row
    groupsContainer = addRow(groupsContainer, true, "Name", group.name);
...
function addChevron(row) {
  ...
  chevronRight.src = "images/chevron-right.svg";

  chevronDown = document.createElement("img");
  chevronDown.className = "chevron-down";
  chevronDown.src = "images/chevron-down.svg";
